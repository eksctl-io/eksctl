...
var _ = Describe("create iamserviceaccount", func() {
	DescribeTable("create service account successfully",
		...
		Entry("with optional flags", "--cluster", "clusterName", "--name", "serviceAccountName", "--attach-policy-arn", "dummyPolicyArn", "--override-existing-serviceaccounts", "--rolename", "custom-role-name"),
	)
