	...
	Describe("Manager", func() {
		Describe("CreateProfile", func() {
			It("fails fast if the provided profile is nil", func() {
				manager := fargate.NewWithRetryPolicy(clusterName, &mocks.EKSAPI{}, &retryPolicy)
				waitForCreation := false
				err := manager.CreateProfile(nil, waitForCreation)
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(Equal("invalid Fargate profile: nil"))
			})

			It("creates the provided profile without tag", func() {
				manager := fargate.NewWithRetryPolicy(clusterName, mockForCreateFargateProfileWithoutTag(), &retryPolicy)
				waitForCreation := false
				err := manager.CreateProfile(createProfileWithoutTag(), waitForCreation)
				Expect(err).To(Not(HaveOccurred()))
			})

			It("creates the provided profile", func() {
				manager := fargate.NewWithRetryPolicy(clusterName, mockForCreateFargateProfile(), &retryPolicy)
				waitForCreation := false
				err := manager.CreateProfile(testFargateProfile(), waitForCreation)
				Expect(err).To(Not(HaveOccurred()))
			})

			It("fails by wrapping the root error with some additional context for clarity", func() {
				manager := fargate.NewWithRetryPolicy(clusterName, mockForFailureOnCreateFargateProfile(), &retryPolicy)
				waitForCreation := false
				err := manager.CreateProfile(testFargateProfile(), waitForCreation)
				Expect(err).To(HaveOccurred())
			...
			It("waits for the full creation of the profile when configured to do so", func() {
				...
				manager := fargate.NewWithRetryPolicy(clusterName, mockForCreateFargateProfileWithWait(numRetriesAfterCreation), retryPolicy)
		...
		Describe("ReadProfiles", func() {
			It("returns all Fargate profiles", func() {
				manager := fargate.NewWithRetryPolicy(clusterName, mockForReadProfiles(), &retryPolicy)
				out, err := manager.ReadProfiles()
				Expect(err).To(Not(HaveOccurred()))
			...
			It("returns an empty array if no Fargate profile exists", func() {
				manager := fargate.NewWithRetryPolicy(clusterName, mockForEmptyReadProfiles(), &retryPolicy)
				out, err := manager.ReadProfiles()
				Expect(err).To(Not(HaveOccurred()))
				Expect(out).To(Not(BeNil()))
			...
			It("fails by wrapping the root error with some additional context for clarity", func() {
				manager := fargate.NewWithRetryPolicy(clusterName, mockForFailureOnReadProfiles(), &retryPolicy)
				out, err := manager.ReadProfiles()
				Expect(err).To(HaveOccurred())
		...
		Describe("ReadProfile", func() {
			It("returns the Fargate profile matching the provided name, if any", func() {
				manager := fargate.NewWithRetryPolicy(clusterName, mockForReadProfile(), &retryPolicy)
				out, err := manager.ReadProfile(testGreen)
				Expect(err).To(Not(HaveOccurred()))
			...
			It("returns a 'not found' error if no Fargate profile matched the provided name", func() {
				manager := fargate.NewWithRetryPolicy(clusterName, mockForEmptyReadProfile(), &retryPolicy)
				out, err := manager.ReadProfile(testRed)
				Expect(err).To(HaveOccurred())
		...
		Describe("DeleteProfile", func() {
			It("fails fast if the provided profile name is empty", func() {
				...
				err := manager.DeleteProfile("", waitForDeletion)
				Expect(err).To(HaveOccurred())
			...
			It("deletes the profile corresponding to the provided name", func() {
				...
				manager := fargate.NewWithRetryPolicy(clusterName, mockForDeleteFargateProfile(profileName), &retryPolicy)
				waitForDeletion := false
				err := manager.DeleteProfile(profileName, waitForDeletion)
				Expect(err).To(Not(HaveOccurred()))
			})

			It("fails by wrapping the root error with some additional context for clarity", func() {
				profileName := "test-green"
				manager := fargate.NewWithRetryPolicy(clusterName, mockForFailureOnDeleteFargateProfile(profileName), &retryPolicy)
				waitForDeletion := false
				err := manager.DeleteProfile(profileName, waitForDeletion)
				Expect(err).To(HaveOccurred())
			...
			It("waits for the full deletion of the profile when configured to do so", func() {
				...
				manager := fargate.NewWithRetryPolicy(clusterName, mockForDeleteFargateProfileWithWait(profileName, numRetriesBeforeDeletion), retryPolicy)
