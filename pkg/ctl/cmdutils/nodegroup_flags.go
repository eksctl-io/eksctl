...
func AddCommonCreateNodeGroupFlags(fs *pflag.FlagSet, cmd *Cmd, ng *api.NodeGroup) {
	...
	fs.StringToStringVar(&ng.Labels, "node-labels", nil, `A list of comma separated KV pairs "k1=v1,k2=v2" as extra labels to add when registering the nodes in the nodegroup, e.g. "partition=backend,nodeclass=hugememory"`)
