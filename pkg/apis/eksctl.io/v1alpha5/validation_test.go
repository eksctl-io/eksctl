...
var _ = Describe("PodIdentityAssociation validation", func() {
	Describe("ValidatePodIdentityAssociation", func() {
		It("should pass for a valid pod identity association", func() {
			pia := &api.PodIdentityAssociation{
				Namespace:          "default",
				ServiceAccountName: "my-service-account",
				RoleARN:            "arn:aws:iam::123456789012:role/my-role",
			}
			err := api.ValidatePodIdentityAssociation(pia)
			Expect(err).NotTo(HaveOccurred())
		})
		...
		It("should fail if namespace is empty", func() {
			...
			err := api.ValidatePodIdentityAssociation(pia)
			Expect(err).To(MatchError("namespace cannot be empty"))
		})

		It("should fail if serviceAccountName is empty", func() {
			pia := &api.PodIdentityAssociation{
				Namespace: "default",
				RoleARN:   "arn:aws:iam::123456789012:role/my-role",
			}
			err := api.ValidatePodIdentityAssociation(pia)
			Expect(err).To(MatchError("serviceAccountName cannot be empty"))
		})

		It("should pass for a valid cross-account pod identity association", func() {
			pia := &api.PodIdentityAssociation{
				Namespace:          "default",
				ServiceAccountName: "my-service-account",
				RoleARN:            "arn:aws:iam::123456789012:role/my-role",
				TargetRoleARN:      "arn:aws:iam::987654321098:role/target-role",
			}
			err := api.ValidatePodIdentityAssociation(pia)
			Expect(err).NotTo(HaveOccurred())
		})

		It("should fail if targetRoleARN is not a valid ARN", func() {
			pia := &api.PodIdentityAssociation{
				Namespace:          "default",
				ServiceAccountName: "my-service-account",
				RoleARN:            "arn:aws:iam::123456789012:role/my-role",
				TargetRoleARN:      "not-an-arn",
			}
			err := api.ValidatePodIdentityAssociation(pia)
			Expect(err).To(MatchError(ContainSubstring("targetRoleARN")))
			Expect(err).To(MatchError(ContainSubstring("not a valid ARN")))
		})

		It("should fail if source and target roles are in the same account", func() {
			pia := &api.PodIdentityAssociation{
				Namespace:          "default",
				ServiceAccountName: "my-service-account",
				RoleARN:            "arn:aws:iam::123456789012:role/my-role",
			...
			err := api.ValidatePodIdentityAssociation(pia)
			Expect(err).To(MatchError(ContainSubstring("targetRoleARN must be in a different account than roleARN")))
		})
	})
})
