...
func createCapabilityCmd(cmd *cmdutils.Cmd) {
...
func doCreateCapability(cmd *cmdutils.Cmd, capability *api.Capability, attachPolicyStr string) error {
	...
	capabilityCreator := capabilityactions.NewCreator(cmd.ClusterConfig.Metadata.Name, stackManager, clusterProvider.AWSProvider.EKS(), cmd)

	for _, cap := range capabilities {
		logger.Info("creating capability %s", cap.Name)
	}
	return capabilityCreator.Create(ctx, capabilities)
}

func configureCreateCapabilityCmd(cmd *cmdutils.Cmd, capability *api.Capability) {
	...
	cmd.FlagSetGroup.InFlagSet("Capability", func(fs *pflag.FlagSet) {
		...
		fs.StringVar(&capability.DeletePropagationPolicy, "delete-propagation-policy", "RETAIN", "Delete propagation policy")
		fs.StringToStringVar(&capability.Tags, "tags", nil, "Tags to apply to the capability")
		// Note: accessPolicies are configured via config file only
	...
	cmd.CobraCommand.RunE = func(_ *cobra.Command, args []string) error {
		...
		return doCreateCapability(cmd, capability, attachPolicyStr)
	}
