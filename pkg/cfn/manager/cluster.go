...
func (c *StackCollection) createClusterTask(errs chan error, supportsManagedNodes bool) error {
	...
	stack := builder.NewClusterResourceSet(c.provider.EC2(), c.provider.Region(), c.spec, supportsManagedNodes, nil)
...
func (c *StackCollection) AppendNewClusterStackResource(plan, supportsManagedNodes bool) (bool, error) {
	...
	newStack := builder.NewClusterResourceSet(c.provider.EC2(), c.provider.Region(), c.spec, supportsManagedNodes, &currentResources)
