	...
	Describe("TaskTree", func() {
		...
		Context("With real tasks", func() {
			...
			It("should have nice description", func() {
				...
				// in these tests
				{
					tasks := stackManager.NewTasksToCreateNodeGroups(makeNodeGroups("bar", "foo"), true, false)
					Expect(tasks.Describe()).To(Equal(`2 parallel tasks: { create nodegroup "bar", create nodegroup "foo" }`))
				}
				{
					tasks := stackManager.NewTasksToCreateNodeGroups(makeNodeGroups("bar"), false, false)
					Expect(tasks.Describe()).To(Equal(`1 task: { create nodegroup "bar" }`))
				}
				{
					tasks := stackManager.NewTasksToCreateNodeGroups(makeNodeGroups("foo"), true, false)
					Expect(tasks.Describe()).To(Equal(`1 task: { create nodegroup "foo" }`))
				}
				{
					tasks := stackManager.NewTasksToCreateNodeGroups(nil, false, false)
