...
var _ = Describe("scale node group config file loader", func() {
	...
	When("scaling a single nodegroup", func() {
	...
	When("scaling all nodegroups", func() {
		When("using a config file", func() {
			It("finds all nodegroups", func() {
				cmd := &Cmd{
					CobraCommand:      newCmd(),
		...
		When("not using a config file", func() {
			It("fails", func() {
				cmd := &Cmd{
					CobraCommand: newCmd(),
				}
				...
				err := NewScaleAllNodeGroupLoader(cmd).Load()
				Expect(err).To(HaveOccurred())
				Expect(err).To(MatchError("a config file is required when scaling multiple nodegroups"))
			})
		})
	})
})
