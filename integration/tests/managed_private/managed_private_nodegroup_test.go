...
func init() {
	...
	testing.Init()
	params = tests.NewParams("managed-private")
}
...
var _ = Describe("(Integration) Create Managed Nodegroups with private networking", func() {

	const (
		initialNodeGroup = "managed-private-ng-0"
		newNodeGroup     = "private-ng-1"
	)

	defaultTimeout := 20 * time.Minute

	Describe("when creating a cluster with 1 managed and private nodegroup", func() {
		...
		Context("and add a managed private nodegroup", func() {
			It("should not return an error", func() {
				cmd := params.EksctlCreateCmd.WithArgs(
					"nodegroup",
					"--cluster", params.ClusterName,
