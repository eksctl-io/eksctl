...
func (c *ClusterResourceSet) RenderJSON() ([]byte, error) {
	...
	logger.Debug("successfully rendered CloudFormation template for cluster %q", c.spec.Metadata.Name)
...
func (c *ClusterResourceSet) generateKarpenterDiscoveryTagsWithValidation() ([]gfncfn.Tag, error) {
	...
	logger.Debug("generating karpenter.sh/discovery tag with value %q for cluster %q", discoveryValue, c.spec.Metadata.Name)
