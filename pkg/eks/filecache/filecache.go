// Disclaimer. This file has been ported over from https://github.com/kubernetes-sigs/aws-iam-authenticator/blob/master/pkg/token/filecache.go
// Reason for the port is to provide a different filename which is not exported and changeable in aws-iam-authenticator without breaking the API.
// Reason for not writing our own caching is that this has been already implemented by the lovely folks of aws-iam-authenticator and there is
// no need to re-implement the wheel if it's already rolling nicely.
package filecache
...
const cacheFileNameEnv = "EKSCTL_CACHE_FILE"
...
// Provider is a Provider implementation that wraps an underlying Provider
...
func NewFileCacheProvider(clusterID, profile, roleARN string, creds *credentials.Credentials) (Provider, error) {
	if creds == nil {
		return Provider{}, errors.New("no underlying Credentials object provided")
	}
	...
	if info, err := f.Stat(filename); !os.IsNotExist(err) {
		if info.Mode()&0077 != 0 {
			...
			return Provider{}, fmt.Errorf("cache file %s is not private", filename)
		}
		...
		if !ok {
			...
			return Provider{}, fmt.Errorf("unable to read lock file %s: %v", filename, err)
		}
...
func (f *Provider) Retrieve() (credentials.Value, error) {
	...
	if err := writeCacheWhileLocked(filename, cache); err != nil {
...
func (f *Provider) IsExpired() bool {
...
func (f *Provider) ExpiresAt() time.Time {
...
func CacheFilename() string {
	...
	return filepath.Join(UserHomeDir(), ".eksctl", "cache", "credentials.yaml")
}
