...
func (m *Manager) postNodeCreationTasks(clientSet kubernetes.Interface, options CreateOpts) error {
	...
	additionalLabelsTask := m.ctl.CreatePostNodeRegistrationLabelerTask(m.cfg, clientSet)
	if errs := additionalLabelsTask.DoAllSync(); len(errs) > 0 {
		...
		return fmt.Errorf("failed to add additional labels to nodegroups for cluster %q", m.cfg.Metadata.Name)
	}
