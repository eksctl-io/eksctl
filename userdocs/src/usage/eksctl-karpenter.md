...
???+ info
    ...
    You have to be logged out of ECR repositories on helm to be able to pull the OCI artifact by running `helm registry logout public.ecr.aws`, failure to do so will result in a 403 error when trying to pull the chart.
