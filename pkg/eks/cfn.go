...
func (c *ClusterProvider) CreateStack(name string, templateBody []byte, parameters map[string]string, withIAM bool, ignoreExists bool) (<-chan error, <-chan Stack) {
	...
	go func() {
		...
		if err != nil && (!ignoreExists || !utils.HasAwsErrorCode(err, cloudformation.ErrCodeAlreadyExistsException)) {
...
func (c *ClusterProvider) createStackVPC() <-chan error {
	...
	go func() {
		...
		taskErrs, stackChan := c.CreateStack(name, templateBody, nil, false, true)
...
func (c *ClusterProvider) createStackServiceRole() <-chan error {
	...
	go func() {
		...
		taskErrs, stackChan := c.CreateStack(name, templateBody, nil, true, true)
...
func (c *ClusterProvider) createStackDefaultNodeGroup() <-chan error {
	...
	go func() {
		...
		taskErrs, stackChan := c.CreateStack(name, templateBody, c.stackParamsDefaultNodeGroup(), true, true)
